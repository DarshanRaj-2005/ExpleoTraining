...
```ts
  ...
  9  | 
  10 |     let inputBox = await page.locator('#user-message').textContent
  11 |     console.log("The Input Box Text:",inputBox)
  12 | 
> 13 |     await page.locator('#user-message').fill('Darshan')
     |                                         ^ Error: locator.fill: Error: strict mode violation: locator('#user-message') resolved to 3 elements:
  14 |     await page.locator('#showInput').click()
  15 | 
  16 |     let mes2 = await page.locator('#message').textContent()
