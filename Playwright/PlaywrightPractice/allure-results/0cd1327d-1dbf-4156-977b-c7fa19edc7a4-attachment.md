...
- Name: TestSkipPractice.test.ts >> Valid Login Test >> Login3
- Location: tests\TestSkipPractice.test.ts:35:5
...
```ts
  ...
  11 |     await page.fill("#loginusername","Darshan")
...
> 46 |     await expect(page.locator("#nameofuser")).toHaveText("Welcome DarshanRaj")
  ...
  61 | 
  62 |     page.on("dialog", async alert => {
  63 |         console.log("Message:",alert.message())
  64 |         await alert.accept()
  65 |     })
  66 | 
  67 |     await page.locator("//button[text()='Log in']").click()
  68 | })
  69 | })
```