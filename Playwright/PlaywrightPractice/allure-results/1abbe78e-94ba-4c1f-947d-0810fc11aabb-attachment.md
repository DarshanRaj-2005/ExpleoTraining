...
```ts
  ...
  11 | 
  12 |     await page.locator("//input[@id='user-message']").fill('Darshan')
  13 |     await page.getByRole('button',{name:'Get Checked Value'}).click()
  14 | 
> 15 |     let mes2 = await expect(page.locator("//p[@id='message']")).toHaveText("Darshan")
     |                                                                 ^ Error: expect(locator).toHaveText(expected) failed
  16 |     await page.locator('mes2').innerText()
  17 |     console.log("The New Message:",mes2)
  18 | })
```