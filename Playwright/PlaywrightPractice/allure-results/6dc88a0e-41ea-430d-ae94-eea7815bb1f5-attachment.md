...
Call log:
  ...
  - waiting for locator('#message')
    6 × locator resolved to <p id="message" class="mt-20"></p>
      - unexpected value ""
...
```ts
  ...
  9  | 
  10 |     await page.locator("//input[@id='user-message']").fill('Darshan')
  11 |     await page.getByRole('button',{name:'Get Checked Value'}).click()
  12 | 
> 13 |     let mes2 = await expect(page.locator('#message')).toContainText("Darshan")
     |                                                       ^ Error: expect(locator).toContainText(expected) failed
  14 |     await page.locator('mes2').textContent()
  15 |     console.log("The New Message:",mes2)
  16 | })
```