...
```ts
  ...
  13 |     await page.locator("//form[@id='gettotal']/button").dblclick()
  14 |     let expected = "40"
> 15 |     await expect(page.locator("//p[@id='addmessage']")).toHaveText("40")
     |                                                         ^ Error: expect(locator).toHaveText(expected) failed
  16 |     await expect(await page.locator("//p[@id='addmessage']").textContent()).toContain(expected)
  17 | })
```