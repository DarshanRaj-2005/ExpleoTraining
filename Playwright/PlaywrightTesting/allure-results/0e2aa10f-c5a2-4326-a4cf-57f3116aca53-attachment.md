...
- Name: DemBlaLoginTest.test.ts >> Valid Login
- Location: tests\DemBlaLoginTest.test.ts:3:5
...
Call log:
  ...
  - waiting for locator('#nameofuser')
    10 × locator resolved to <a href="#" id="nameofuser" class="nav-link"></a>
...
> 9  |     await expect(page.locator("#nameofuser")).toBeVisible();
     |                                               ^ Error: expect(locator).toBeVisible() failed
  10 | 
  11 |     console.log("Title:",await page.title())
  12 |     console.log("Url:",await page.url())
  13 |     console.log("Content:",await page.content())
  14 | })
```